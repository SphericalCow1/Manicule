...
pub(crate) fn create_page_in_workspace(
    ...
    let page = workspace
        ...
        .ok_or_else(|| {
            AppError::internal(
                "The page was created, but mentiNote could not add it to the workspace index. Refresh the workspace.",
...
pub(crate) fn move_page_in_workspace(
    ...
    let page = workspace
        ...
        .ok_or_else(|| {
            AppError::internal(
                "The page was moved, but mentiNote could not update the workspace index. Refresh the workspace.",
...
pub(crate) fn rename_page_in_workspace(
    ...
    let page = workspace
        ...
        .ok_or_else(|| {
            AppError::internal(
                "The page was renamed, but mentiNote could not update the workspace index. Refresh the workspace.",
...
fn move_folder_to_path(
    ...
    for page in &pages_to_rename {
        let suffix = page.path.strip_prefix(&old_folder_prefix).ok_or_else(|| {
            AppError::internal(
                "mentiNote could not prepare the folder move. Refresh the workspace and try again.",
                format!(
...
mod tests {
    ...
    fn temp_workspace() -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!("mentinote-page-ops-test-{now}-{counter}"));
