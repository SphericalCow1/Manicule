...
use mentinote_lib::app_state::WorkspaceState;
use mentinote_lib::content_snapshot::ContentSnapshot;
use mentinote_lib::dto::SavePageResultDto;
use mentinote_lib::index::backlink_index::BacklinkIndex;
use mentinote_lib::index::page_index::PageIndex;
use mentinote_lib::page_io::{content_hash, save_page_in_workspace};
use mentinote_lib::query::{list_tasks_in_workspace, search_pages_in_workspace};
use mentinote_lib::workspace_config::WorkspaceConfig;
use mentinote_lib::workspace_index::{reindex_workspace, reindex_workspace_paths};
...
fn run_benchmarks(
    ...
    println!("mentiNote indexing and query benchmark");
...
fn create_workspace(config: &BenchmarkConfig) -> Result<GeneratedWorkspace, String> {
    let now = SystemTime::now()
        ...
        .as_nanos();
    let root = env::temp_dir().join(format!("mentinote-performance-benchmark-{now}"));
